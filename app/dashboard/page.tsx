...
export default function Dashboard() {
  ...
  const handleEnrollCourse = async (course: Course) => {
    ...
    try {
      ...
      setStudentData((prev) => prev && {
        ...
        courses: [
          ...(prev.courses || []),
          { ...newCourse, id: "", resources: [], tests: [] }, // Ensure it matches the Course type
        ],
      });
  ...
  const handleAddSubject = async (studentId: string, courseName: string, subjectName: string) => {
    ...
    const courses = student.courses.map((c) =>
      c.name === courseName
        ? { ...c, subjects: [...(c.subjects || []), { name: subjectName, grades: { C1: "", C2: "", exam: "", final: "" }, comments: "" }] }
        : c
    );
    try {
  ...
  const handleSendNotification = async (studentId: string, message: string) => {
    ...
    try {
      ...
      setAllStudents((prev) =>
        prev.map((s) =>
          s.id === studentId
            ? { ...s, notifications: [...(s.notifications || []), { ...notif, id: docRef.id }] }
            : s
        )
      );
  ...
  const handleUploadResource = async () => {
    if (!["teacher", "admin"].includes(role) || !newResource.courseId || !newResource.name || !newResource.type || !newResource.url)
      return alert("Fill all resource fields");
    try {
      ...
      setAllCourses((prev) =>
        prev.map((c) => (c.id === newResource.courseId ? { ...c, resources: [...(c.resources || []), resource] } : c))
      );
  ...
  const handleCreateTest = async () => {
    if (
      ...
      !newTest.courseId ||
      !newTest.title ||
      newTest.questions.some((q) => !q.question || !q.correctAnswer || (q.options.length > 1 && q.options.some((o) => !o)))
    )
    ...
    try {
      ...
      setAllCourses((prev) =>
        prev.map((c) => (c.id === newTest.courseId ? { ...c, tests: [...(c.tests || []), test] } : c))
      );
  ...
  if (error)
    return (
      <div className="text-red-800 text-center">
        <p>{error}</p>
        <button onClick={() => window.location.reload()} className="mt-2 px-4 py-1 bg-red-800 text-white rounded hover:bg-red-700">
          Retry
        </button>
      </div>
    );
        ...
        <div className="max-w-7xl mx-auto">
          ...
          {role === "student" && (
            <div className="space-y-6">
              {!studentData ? (
                <p className="text-red-800 text-center bg-white p-4 rounded shadow">
                            ...
                            {!n.read && (
                              <button
                                onClick={() =>
                                  user &&
                                  n.id &&
                                  markNotificationAsRead(user.uid, n.id).then(() =>
                                    setStudentData((prev) =>
                                      prev && {
                                        ...prev,
                              ...
                              <tbody>
                                {(c.subjects || []).map((s) => (
                                  <tr key={s.name}>
                                    ...
                                    <td className="p-1 border text-red-800">
                                    ...
                                    <div key={t.id} className="mt-2">
                                      ...
                                      {testResponses[t.id]?.submittedAt ? (
                                        <p className="text-red-800">
                                          Submitted: {new Date(testResponses[t.id].submittedAt!).toLocaleString()} (Score:{" "}
                        ...
                        {studentData.transactions.length ? (
                          studentData.transactions.map((t) => (
                            <p key={t.id || t.date} className="text-red-800">
                              {new Date(t.date).toLocaleString()}: {t.amount.toLocaleString()} JMD - {t.status}
                            </p>
                          ))
                        ) : (
                            ...
                            {s.notifications?.length ? (
                              s.notifications.map((n) => (
                                <p key={n.id || n.date} className="text-red-800">
                                  {new Date(n.date).toLocaleString()}: {n.message} ({n.read ? "Read" : "Unread"})
                                </p>
                              ))
                            ) : (
                              <p className="text-red-800">No notifications</p>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <div className="space-y-4">
                  <div className="bg-white p-4 rounded shadow">
                    ...
                    <p className="text-red-800">
                      Revenue: {allStudents.reduce((sum, s) => sum + s.totalPaid, 0).toLocaleString()} JMD
                    </p>
                  </div>
                  <div className="bg-white p-4 rounded shadow">
                    ...
                    <button
                      onClick={() =>
                        handleAddCourse(
                          ...
                          parseFloat(
                            (document.getElementById("new-course-fee") as HTMLInputElement)?.value || "0"
                          )
                        )
                      }
                      className="px-4 py-1 bg-red-800 text-white rounded hover:bg-red-700"
                    ...
                    allStudents.map((s) => (
                      <div key={s.id} className="bg-white p-4 rounded shadow">
                        ...
                        <div className="mt-2">
                          ...
                          <select
                            value={s.lecturerId || ""}
                            onChange={(e) => handleAddStudent(s.name, s.email, e.target.value)}
